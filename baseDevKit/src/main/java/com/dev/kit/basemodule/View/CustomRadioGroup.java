...
/**
 * Created by cuiyan on 15/11/2 19:12
...
public class CustomRadioGroup extends LinearLayout {
    ...
    public static final int mInvalidCheckedId = -1;
    ...
    public CustomRadioGroup(Context context, AttributeSet attrs) {
        ...
        final int index = attributes.getInt(R.styleable.CustomRadioGroup_orientation, VERTICAL);
        if (index == LinearLayout.HORIZONTAL) {
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        if (child instanceof RadioButton) {
            ...
            checkAddedView((ViewGroup) child);
        }
    ...
    private void checkAddedView(ViewGroup vg) {
        ...
        for (int i = 0; i < len; i++) {
            if (vg.getChildAt(i) instanceof RadioButton) {
                ...
                checkAddedView((ViewGroup) vg.getChildAt(i));
            }
        }
    }
...
//                    id = View.generateViewId();
                ...
                findRadioButton((ViewGroup) child, list);
                if (list.size() > 0) {
        ...
        public void onChildViewRemoved(View parent, View child) {
            if (parent == CustomRadioGroup.this && child instanceof RadioButton) {
                ...
                mRadioButtonList.remove(((RadioButton) child));
            } else if (child instanceof ViewGroup) {
                List<RadioButton> list = new ArrayList<>();
                findRadioButton((ViewGroup) child, list);
                if (list.size() > 0) {
                    mRadioButtonList.removeAll(list);
                    for (RadioButton button : list) {
    ...
    /**
     * 遍历指定ViewGroup下所有RadioButton
     * @param group 待遍历的ViewGroup
     * @param list 将查找到的RadioButton放入列表中
     */
    public void findRadioButton(ViewGroup group, List<RadioButton> list) {
        int len = group.getChildCount();
        for (int i = 0; i < len; i++) {
            if (group.getChildAt(i) instanceof RadioButton) {
                list.add((RadioButton) group.getChildAt(i));
            } else if (group.getChildAt(i) instanceof ViewGroup) {
                findRadioButton((ViewGroup) group.getChildAt(i), list);
            }
        }
    }
}
